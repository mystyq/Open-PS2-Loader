...
static int ata_wait_ready(int timeout_ms)
{
    USE_ATA_REGS;
    int waited = 0;
    for (;;) {
        ...
        if (waited >= timeout_ms) {
            M_PRINTF("ATA: wait_ready TIMEOUT after %d ms (BUSY=0x%02x)\n", waited, status);
...
/* Wait until the ATA device becomes ready.
   SATA: BUSY=0 and DRDY=1
   PATA: BUSY=0 (DRDY optional)
*/
static int ata_wait_for_device_ready(int timeout_ms)
{
    USE_ATA_REGS;
    int waited = 0;
    for (;;) {
        u8 status = ata_hwport->r_status;
        if (!(status & ATA_STAT_BUSY)) {                 /* BUSY must be clear for all devices */
            if (!lba_48bit || (status & ATA_STAT_READY)) /* SATA requires DRDY */
                return 0;                                /* Ready */
        }
        if (waited++ >= timeout_ms)
            return ATA_RES_ERR_TIMEOUT;
...
int atad_start(void)
{
    ...
    /* BUSY/DRDY readiness (SATA strict, PATA tolerant) */
    res = ata_wait_for_device_ready(30000);
    if (res < 0) {
        M_PRINTF("ATA: Stage 1 readiness failed (BUSY/DRDY spin-up), res=%d\n", res);
        goto out;
    }
    /* ATA-spec wait */
    res = ata_wait_ready(5000);
    if (res < 0) {
        M_PRINTF("ATA: Stage 2 readiness failed (ATA-spec DRDY/ERR), res=%d\n", res);
        goto out;
    }
    res = 0;
